...
function renderHabits(){
  ...
  [...FIXED_HABITS,...extras].forEach((h)=>{const cb=el('input',{type:'checkbox',checked:done.has(h),onchange(){this.checked?done.add(h):done.delete(h);LS.set(key,[...done])}}); list.append(el('div',{class:'item'},[el('label',{},[h]),cb]))});
  wrap.append(list);
...
function evaluateDay({energia,sono,humor,foco,dores}){
  ...
  if (dores >= 4){
    ...
    nivel='Alta'; tipo='Força/Metabólico';
    msg='🔥 Pico de performance! Trabalhe cargas progressivas em movimentos compostos. Finalize com técnica sob fadiga controlada.';
  } else if (media >= 3.5){
    nivel='Moderada-alta'; tipo='Treino completo — técnica + carga';
    msg='Dia bom! Execute o plano com foco na forma e aumentos sutis de carga.';
  } else if (media >= 2.5){
    nivel='Moderada'; tipo='Técnica e controle de cadência';
    msg='Mantenha a constância. Priorize execução perfeita e cadência 3-1.';
  } else if (media >= 1.5){
    nivel='Leve'; tipo='Cardio leve + mobilidade';
    msg='Energia baixa. Faça 20–30min de caminhada/ciclismo leve e mobilidade.';
  } else {
    nivel='Recuperativo'; tipo='Respiração/Meditação/Descanso ativo';
