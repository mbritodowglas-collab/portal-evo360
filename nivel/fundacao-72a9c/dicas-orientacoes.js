...
<head>
  ...
  <style>
    ...
    /* Tabs — tudo numa linha, com rolagem lateral se precisar */
    .tabs{
      display:flex;
      gap:8px;
      margin:12px 0;
      flex-wrap:nowrap;                 /* mesma linha */
      overflow-x:auto;                  /* rola se faltar espaço */
      -webkit-overflow-scrolling:touch; /* rolagem suave no iOS */
      scrollbar-width:none;             /* esconde barra no Firefox */
    }
    .tabs::-webkit-scrollbar{ display:none; } /* esconde barra no WebKit */

    .tab{
      flex:0 0 auto;                    /* largura do conteúdo */
      padding:8px 12px;
      border-radius:10px;
    ...
    /* 🔒 Esconde o contador do formulário de meta (mantemos só nos cards) */
  ...
  (function(){
    ...
    function migrateToLocalKeys(){
      ...
      HABITS.forEach(h=>{
        ...
        for(const [k,v] of Object.entries(hist)){ if(isLocal(k)){ newer[k]=v; continue; } const loc=localISO(new Date(k)); newer[loc]=v; changed=changed||loc!==k; }
        if(changed) LS.set(keyHabit(h.id),newer);
      });
      const dcomp=LS.get(KEY_DAYCOMP,null);
      if(dcomp){ let changed=false; const newer={}; for(const [k,v] of Object.entries(dcomp)){
